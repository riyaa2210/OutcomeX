      ...
      {file ? (
        <>
          ...
          <button onClick={e => { e.stopPropagation(); onClear(); }}
            className="absolute right-3 top-3 rounded-full p-1.5 text-slate-500
        ...
        <>
          ...
          <div>
            ...
            <p className="text-xs text-slate-600 mt-0.5">.mp3 · .wav · .m4a — or click to browse</p>
          </div>
        </>
      )}
    </div>
    ...
    <M.button
      ...
      style={disabled
        ...
        : { background: "linear-gradient(135deg, #4338ca 0%, #0e7490 100%)",
            boxShadow: "0 4px 28px rgba(67,56,202,0.38)" }}
    >
      <span className="relative flex items-center justify-center gap-2">
        {loading ? (
          ...
          <><Zap className="h-4 w-4" /> Analyse Meeting</>
        )}
      </span>
    </M.button>
  );
...
function StatPills({ structured }) {
  ...
  return (
    <M.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.3, ease: easeSoft }} className="flex gap-2">
      {stats.map(s => (
...
function Header() {
  return (
    <div className="mb-8 flex items-center gap-4">
      ...
      <div>
        <h1 className="text-xl font-black leading-tight tracking-tight text-slate-100">
    ...
    try {
      if (file) {
        ...
        if (!prRes.ok) {
          ...
          if (prRes.status === 503) throw new Error(err.detail || "Audio transcription unavailable. Please paste your transcript instead.");
        ...
        <div className="w-full lg:w-5/12 space-y-3">
          <div className="rounded-3xl p-5 space-y-4"
            ...
            <div className="flex items-center gap-2">
              <span className="text-xs font-black uppercase tracking-widest text-slate-700">Input</span>
          ...
          <AnimatePresence>
            {step >= 0 && (
              <M.div key="tracker" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
